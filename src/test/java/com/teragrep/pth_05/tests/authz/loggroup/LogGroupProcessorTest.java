...
package com.teragrep.pth_05.tests.authz.loggroup;
...
public class LogGroupProcessorTest {
    ...
    public void loadTest() throws IOException {
        ...
        LinkedList<LogGroup> logGroupList = logGroupProcessor.load("src/test/resources/lookup");

        // CFE-12 lookups have no way to determine all groups for host, iterating all
        String index = null;
        for (LogGroup group : logGroupList) {
            final String tmp = group.getIndex("example-host-one",
                    "example-tag-one"
            );
        ...
        Assertions.assertEquals("example-tag-one", index);
    }
}
